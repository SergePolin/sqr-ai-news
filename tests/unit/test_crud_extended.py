...
from app.db.models import NewsArticle, User, UserChannels, Bookmark
...
def test_get_articles_with_filters(test_db: Session):
    ...
    logger.debug("Getting articles filtered by source='tech_source' and category='Technology'")
    ...
    if tech_category_articles:
        logger.debug(f"Article sources and categories: {[(a.source, a.category) for a in tech_category_articles]}")
    
    assert len(tech_category_articles) == 2
    assert all(a.source == "tech_source" and a.category == "Technology" 
...
def test_user_channel_operations(test_db: Session, sample_user):
    ...
    logger.debug(f"Retrieving specific channel with ID: {channel_id}, Type: {type(channel_id)}")
    
    # Using SQLAlchemy directly with raw SQL to work around UUID issues
    try:
        result = test_db.execute(
            text(f"SELECT * FROM user_channels WHERE id = :id"),
            {"id": str(channel1.id)}
        ).fetchall()
        logger.debug(f"Raw SQL result for channel {channel_id}: {result}")
        
        # Now use the CRUD function for assertion
        retrieved_channel = get_channel(test_db, channel1.id)  # Pass the UUID object directly
    ...
    try:
        updated_channel = update_channel(test_db, channel1.id, "@updated_channel")  # Pass the UUID object directly
    ...
    if remaining_channels:
        logger.debug(f"Remaining channel aliases: {[c.channel_alias for c in remaining_channels]}")
