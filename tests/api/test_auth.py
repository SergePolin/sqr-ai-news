...
def test_register_duplicate_username(test_db: Session, clean_db):
    ...
    if user:
        logger.debug(
            f"User ID: {user.id}, Username: {user.username}, Email: {user.email}"
        )
    ...
    logger.debug(
        f"Attempting to register duplicate username with data: {duplicate_request}"
    )

    response = client.post("/auth/register", json=duplicate_request)
...
def test_login_wrong_password(clean_db):
    ...
    response = client.post(
        "/auth/login", data={"username": username, "password": "wrongpassword"}
    )
...
def test_login_nonexistent_user():
    ...
    response = client.post(
        "/auth/login",
        data={"username": f"nonexistent_user_{unique_id}", "password": "password123"},
    )
    assert response.status_code == 401
    assert "Incorrect username or password" in response.json()["detail"]
...
def test_password_hashing(test_db: Session, clean_db):
    ...
    response_incorrect = client.post(
        "/auth/login", data={"username": username, "password": "wrong_password"}
    )
